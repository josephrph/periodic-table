...
- ~~**OPEN (found 2026-08-12):** `CITATIONS` contains 10 exact-duplicate entries~~ — **RESOLVED** in `7c5f933` (2026-08-12) and **re-verified 2026-09-03**: all ten named pairs (CBG/`33562819`, MY/`37764505`, bCA/`30166097`, LI/`34362338`, aHU/`31259712`, TE/`24084350`, TE/`35704944`, GE/`21371438`, FA/`41739313`, aBI/`36854520`) now appear **exactly once**, each retaining the condition-tagged copy. This line previously contradicted §10, which correctly recorded the fix.
- **⚠ OPEN (found 2026-09-03, NOT fixed — a NEW instance of the same defect class, not the old ten):** three PMIDs are duplicated **within a single molecule** and therefore render more than once in the molecule panel and the molecule-profile printout. `CITATIONS[id] || []` takes every entry and **nothing filters by the `indications` tag** (verified), so each copy renders.
  - **CBD / `41856154`** (Wilson 2026, Lancet Psychiatry) — **3 copies**, tagged Opioid Use Disorder / Cannabis Use Disorder / Bipolar Disorder
  - **CBD / `37223912`** (Jefsen 2023, JAMA Psychiatry) — **2 copies**, tagged Bipolar Disorder / Antidepressant
  - **THCA / `33341026`** (Carmona-Hidalgo 2021, Phytomedicine) — **2 copies**, one untagged base + one tagged Chronic Liver Disease; the untagged copy also has a truncated title (missing the `Δ⁹-` prefix)
  **The two CBD cases are NOT blind duplicates** — each copy carries a genuinely different condition-specific `note`, so deduplicating without merging would DELETE content. The THCA case is the classic pattern and matches the ten fixed in `7c5f933` exactly.
  **Root cause, which `7c5f933` did not address:** the init step concatenates **five** topic blocks onto the base — `CANCER_CITATIONS`, `LIVER_CITATIONS`, `OUD_CITATIONS`, `MENTAL_CITATIONS`, `PCOS_CITATIONS`. The August fix removed the ten CANCER collisions but not the mechanism; the four later blocks can each recreate it, and three did. **A preflight guard failing on any within-molecule duplicate PMID would close this permanently** and is the durable fix. Recommended minimum correction: remove the untagged THCA base copy (as `7c5f933` did), **merge** rather than delete the two CBD sets, and add the guard.
